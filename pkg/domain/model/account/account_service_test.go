...
func TestAccountService_GetToken(t *testing.T) {
	type fields struct {
	...
	type args struct {
		...
		tx  *gorm.DB
		rtx redis.Pipeliner
		req *AccountGetTokenRequest
	}
	tests := []struct {
		name    string
		fields  fields
		args    args
		want    *AccountGetTokenResponse
		wantErr error
	}{
		{
			name: "正常：確認できる場合",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := s.GetToken(tt.args.ctx, tt.args.req)
			if !reflect.DeepEqual(err, tt.wantErr) {
...
func TestAccountService_CreateUserId(t *testing.T) {
	type fields struct {
		userAccountMysqlRepository func(ctrl *gomock.Controller) userAccount.UserAccountMysqlRepository
		shardService               func(ctrl *gomock.Controller) shard.ShardService
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := s.CreateUserId(tt.args.ctx)
			if !reflect.DeepEqual(err, tt.wantErr) {
				t.Errorf("CreateUserId() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if err == nil {
			...
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("CreateUserId() = %v, want %v", got, tt.want)
			}
		})
	}
}
